...
class MemoriesByMomentScreen extends StatefulWidget {
  final MemorialResponseModel memorial;

  const MemoriesByMomentScreen({
    Key? key,
    required this.memorial,
  }) : super(key: key);

  @override
  State<MemoriesByMomentScreen> createState() => _MemoriesByMomentScreenState();
}

class _MemoriesByMomentScreenState extends State<MemoriesByMomentScreen> {
  ...
  Future<void> _loadMemories() async {
    ...
    try {
      final response = await _memoryService.getMemoriesGroupedByMoment(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Recuerdos por Momento'),
          ...
          : _groupedMemories.isEmpty
              ? const Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text(
                        'No hay recuerdos organizados por momento',
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final moment = _groupedMemories.keys.elementAt(index);
                    final typeMap = _groupedMemories[moment]!;

                    return _buildMomentSection(moment, typeMap);
                  },
                ),
    );
  }
  ...
  Widget _buildMomentSection(
      String moment, Map<String, List<MemoryLiteResponse>> typeMap) {
    // Contar total de memorias en este momento
    int totalCount = 0;
